...
const testUserModel = async () => {
  try {
    ...
    const foundByEmail = await UserModel.findByEmail('test-model@example.com');
    ...
    const students = await UserModel.findAll({ role: 'student' });
    ...
    // Verify deletion
    const deletedUser = await UserModel.findById(newUser.id);
    if (!deletedUser) {
